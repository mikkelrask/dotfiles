...
handle() {
  case $1 in
    windowtitle*)
      ...
      if [[ "$window_title" == *"Bitwarden — LibreWolf"* ]]; then
        ...
        hyprctl --batch \
          "dispatch togglefloating address:0x$window_id ; \
           dispatch resizewindowpixel exact 500 900,address:0x$window_id ; \
           dispatch movewindowpixel exact 4312 807,address:0x$window_id"
      fi
      ;;
  esac
}
